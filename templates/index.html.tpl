...
.card-image {
  ...
  /* scale된 자식이 둥근 클립을 벗어나는 합성 글리치 방지 — clip-path가 합성 레이어도 확실히 자름 */
  clip-path: inset(0 round 13px 13px 0 0);
}
...
/* 즉시 뜨는 힌트 툴팁 (DRAFT / DATA LOW) — body 직속이라 카드 클리핑 없음 */
.hint-tip {
  position: fixed;
  z-index: 2000;
  background: var(--ink);
...
(function(){
  ...
  // ─── 즉시 툴팁 (네이티브 title ~1초 지연 회피) — DRAFT / DATA LOW 배지 ───
  (function(){
    let tip = null;
    function ensure(){
      if (!tip){ tip = document.createElement('div'); tip.className = 'hint-tip'; document.body.appendChild(tip); }
      return tip;
    }
    function hide(){ if (tip) tip.classList.remove('show'); }
    document.querySelectorAll('.tag-draft[title], .card-warning[title]').forEach(el => {
      el.dataset.tip = el.getAttribute('title');
      el.removeAttribute('title');  // 네이티브 지연 툴팁 제거 (중복 방지)
  ...
  function applyFilters() {
    ...
    document.querySelectorAll('.card').forEach(card => {
      ...
      const show = tierOk && cityOk && typeOk && model3dOk && searchOk;
